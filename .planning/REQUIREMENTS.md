...
- [x] **PRES-03**: Usuário continua gerando hashes MD5, SHA-1, SHA-256 e SHA-512 com os mesmos resultados válidos após a migração visual.
- [x] **PRES-04**: Usuário não perde funcionalidades existentes enquanto as três telas são migradas em incrementos verificáveis.
...
- [x] **DIAG-06**: ~~Usuário pode executar um teste de alcance e latência do gateway quando houver alvo e método tecnicamente disponíveis.~~ **Descopado na Phase 4 (decisão 2026-09-09; confirmado na validação física de fechamento em 2026-09-19)**: implementado na Phase 3 como probe TCP connect, mas o teste físico mostrou que reportava quase sempre *indisponível* (roteadores filtram portas; TCP connect cru não prova que a página do gateway abre) e confundia o usuário sem agregar valor. Probe removido da UI (check físico PF-1 confirma ausência do probe e presença do endereço); o endereço do gateway (DIAG-04) permanece. Latência útil coberta por DIAG-07 (HTTPS). ICMP fora do escopo do MVP. O histórico da Phase 3 (implementação original) é preservado, não reescrito.
